		...
		methods: {
			applyUploadCertificate() {
				this.request({
					...
					this.header.AccessKey = accessKey;
					this.header.SecretKey = secretKey;
				}).catch(err => {
					uni.showToast({
						icon: 'none',
				...
				uni.chooseImage({
					...
					success: (res) => {
						...
						let uploadTask = uni.uploadFile({
							url: this.baseUrl + `consumer/my/imageRecognition?flag=${flag}`,
							...
							success: (uploadFileRes) => {
								...
								if (resData.code == 0) {
									...
									if (flag == "front") {
										this.consumerDetails.fullname = resData.result.idCardName;
										this.consumerDetails.idNumber = resData.result.idCardNo;
										this.consumerDetails.address = resData.result.idCardAddress;
									}
								} else {
									if (flag == "front") {
					...
					success: (res) => {
						...
						if (resData.flag) {
							if (this.consumerDetails.flag == "front") {
								this.consumerDetails.idCardPhoto = resData.data;
							}
							if (this.consumerDetails.flag == "back") {
								this.consumerDetails.idCardEmblem = resData.data;
							}
						} else {
							if (this.consumerDetails.flag == "front") {
								this.frontImgPath = "../../static/img/identityIcon.png";
