	...
	<view class="content lendDetails">
		...
		<view class="topHead">
				...
				<view class="box">
					...
					<view class="items">
						<view class="progressBar">
							<view class="progress" :style="proWid"></view>
						</view>
					</view>
					<view class="items">
			...
			<view class="cardsBlock infoCont">
				...
				<view class="progressBarX">
					<view class="progressX" style="width: 80%;"></view>
				</view>
				<view class="info">
				...
				<swiper-item>
					<scroll-view class="cont" scroll-y>
						<view class="block">
							...
							<view class="items">
								<view>手机号</view>
								<view>{{utils.numReplaceStar(userInfo.mobile,3,7)}}</view>
							</view>
							<view class="items">
								<view>身份证</view>
								<view>{{utils.numReplaceStar(userInfo.idNumber,4,14)}}</view>
							</view>
					...
					<scroll-view class="cont" scroll-y>
						<view class="block">
							...
							<view class="items" v-for="(item, index) in planData" :key="index">
								<view>{{item.planItem}}期</view>
								<view>{{item.amount.toFixed(2)}}</view>
								<view>{{item.interest.toFixed(2)}}</view>
								<view>{{item.st}}</view>
							</view>
						</view>
					</scroll-view>
		...
		<view class="ButList" v-if="details.remainingAmount > 0">
			<view class="inpbox"><input type="number" class="inpMonay" v-model="money" :placeholder="`金额不得高于${details.remainingAmount}`">
				<text>元</text></view>
		...
		methods: {
			submitHandle: function() {
				if (this.money && 0 < this.money && this.money <= Number(this.details.remainingAmount)) {
					uni.navigateTo({
						url: `/pages/lend/confirmLend?id=${this.ids}&amount=${this.money}&objName=${this.objName}`
					})
				} else {
					uni.showToast({
						icon: 'none',
			...
			computPlanData(dat, amount) {
				...
				for (let i = 1; i <= dat.period / 30; i++) {
					let arr = {
						planItem: i,
						amount: amount / (dat.period / 30),
						interest: amount * dat.annualRate / (dat.period / 30),
						st: '待还'
					}
					planData.push(arr)
				}
